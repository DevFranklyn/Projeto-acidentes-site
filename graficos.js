...
d3.csv(csvUrl, d3.autoType).then(data => {
    ...
    let diaMaisPerigoso = { dia: '-', acidentes: 0 };
    dadosDiaSemana.forEach(d => {
        if(d.acidentes > diaMaisPerigoso.acidentes) {
            diaMaisPerigoso = d;
        }
    });
    ...
    if (kpiContainer) {
        kpiContainer.innerHTML = `
            <div class="kpi-card">
                ...
                <div class="kpi-value">${totalAcidentes.toLocaleString('pt-BR')}</div>
                <div class="kpi-sub">Registros no Dataset</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-title">Estado com Mais Casos</div>
                <div class="kpi-value">${ufMaisPerigosa.uf}</div>
                <div class="kpi-sub">${ufMaisPerigosa.total.toLocaleString('pt-BR')} acidentes</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-title">Dia Mais Crítico</div>
                <div class="kpi-value" style="text-transform: capitalize;">${diaMaisPerigoso.dia}</div>
                <div class="kpi-sub">${diaMaisPerigoso.acidentes.toLocaleString('pt-BR')} acidentes</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-title">Causa Principal</div>
                <div class="kpi-value causa-value">${causaPrincipal.causa}</div>
            </div>
        `;
    }
